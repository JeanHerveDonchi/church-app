...
function Login() {
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      if (error) {
        setFormError(
          getAuthErrorMessage(error, 'Impossible de vous connecter pour le moment.'),
        )
        return
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        setFormError(
          getAuthErrorMessage(error, 'Impossible de lancer la connexion Google.'),
        )
      }
    } finally {
  ...
  const isBusy = isSubmitting || isGoogleSubmitting || isCheckingLifecycle
          ...
          <div className="mx-auto flex w-full max-w-md flex-col gap-6">
            ...
            >
              ...
              <button
                ...
                type="submit"
              >
                {isSubmitting ? 'Connexion...' : isCheckingLifecycle ? 'Vérification...' : 'Se connecter'}
              </button>
            </form>
