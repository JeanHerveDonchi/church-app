...
function ManageUsers() {
  ...
  const [profileToDelete, setProfileToDelete] = useState<SearchableProfile | null>(
    null,
  )
  ...
  const runSearch = async (mode: SearchMode, query: string) => {
    ...
    try {
      ...
      const result =
        mode === 'email'
          ? await queryBuilder.ilike('email', normalizeEmail(query))
          : await queryBuilder.ilike('full_name', `%${query}%`)

      if (result.error) {
        throw result.error
      }

      const nextProfiles = (result.data ?? [])
  ...
  const handleEmailSearch = async () => {
    ...
    await runSearch('email', trimmedEmail)
  }
